...
-   `pagekit_data` → `/var/www/data`: `config.php`, the snapshots removing a package leaves behind and, for an SQLite installation, the database file. The installer writes `config.php` and the settings screens rewrite it while the site runs, so the image ships it as a link into this directory rather than a file of its own. `tmp/snapshots` is a link into it for a related reason: a snapshot is the only copy of a package the installation no longer has, and a removal may not turn irreversible because a newer image replaced the container. A start that cannot make that link, or cannot write through it, is refused rather than allowed to keep snapshots somewhere they will be lost.
...
The application tree, `packages/` included, belongs to root in the image and the site runs as `www-data`, so a running container serves the packages its image was built with and takes no other: the panel's _Upload_ → _Install_ and `php pagekit install <archive.zip>` are refused at the first folder they try to create, a restore is refused before it touches the database, and an uninstall gets as far as its snapshot, the disabling and the package's own cleanup before the folder refuses to go — the package's data is gone, its tree is listed again as installed and off, and the snapshot is the copy to keep. A restore the version check turns away — a different application version, a package list that does not match, a stored version that was lost, or metadata that cannot be read — says so before any file is touched. A snapshot from before that version was recorded can still pass that check when the package list matches. One that passes it then meets the read-only `packages/` tree. Add a package by putting it under `packages/` and building the image again; where a package installed at runtime lives in a container is ROADMAP Step 2.7.5. `php pagekit archive` in the container writes into the installation root by default, which is root's as well; give it `--dir` with a directory the site can write, the data volume (`/var/www/data`) being the one that outlives the container.
...
-   **From the command line**: `php pagekit install <archive.zip>` installs it the same way and does not enable it. `php pagekit enable <package>` switches it on and `php pagekit disable <package>` switches it off — the same checks as the panel, and neither command has `--force`. Enable refuses when a module the package requires is missing or switched off. Disable and uninstall refuse when an enabled module still requires the package. On the extensions page, Disable and Remove stay hidden unless that check reports no such module. On the themes page, Remove stays hidden the same way. The active theme has no Disable control: enabling another theme replaces it and does not ask whether an enabled module still requires the one it replaces. `php pagekit disable` on that theme does ask.
