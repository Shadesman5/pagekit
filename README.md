...
-   **From the command line**: `php pagekit install <archive.zip>` installs it the same way and does not enable it. `php pagekit enable <package>` switches it on and `php pagekit disable <package>` switches it off — the same checks as the panel, and neither command has `--force`. Enable refuses when a module the package requires is missing or switched off. Disable and uninstall refuse when an enabled module still requires the package. On the extensions and themes pages, Disable and Remove stay hidden unless that check reports no such module.
