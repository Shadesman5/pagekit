...
/**
 * Builds every entry of the bundle manifest with Vite.
 *
...
/**
 * @param {string[]} argv
 * @returns {{ watch: boolean, concurrency: number }}
 */
function parseArgs(argv) {
    const options = { watch: false, concurrency: Math.min(availableParallelism(), 8) };
...
async function main() {
    ...
    // Watchers stay resident, so they are started one by one rather than pooled.
    const failures = watch
        ? await runPool(entries, 1, watchEntry)
        : await runPool(entries, concurrency, buildEntry);
    ...
    console.log(`\n${entries.length} bundles ${watch ? 'watched' : 'built'} in ${seconds}s`);
}
