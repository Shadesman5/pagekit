        ...
        <div class="uk-card uk-card-default uk-transition-toggle">
          <div class="uk-card-media-top">
            <div class="uk-inline-clip">
              <div
                class="uk-background-contain uk-position-cover"
                :data-src="p.extra.image"
                uk-img
              />
        ...
        <div class="uk-modal-header uk-position-relative">
          ...
          <ul class="uk-subnav uk-subnav-divider uk-margin-remove-top uk-margin-remove-bottom">
            <li v-if="pkg.author.homepage">
              <a class="uk-link-muted" :href="pkg.author.homepage" target="_blank">{{
                pkg.author.name
              }}</a>
            </li>
              ...
              <div class="uk-position-relative uk-height-1-1">
                <div class="tm-description uk-position-cover">
                  <div class="tm-description-wrapper">
                    <div
                      uk-overflow-auto="selContainer: .tm-description; selContent: .tm-description-wrapper"
                    >
                      <div
                        v-if="pkg.description"
                        class="uk-text-small"
                        v-html="marked(pkg.description)"
                      />
                    </div>
                    <div class="uk-margin-top">
                      <ul class="uk-grid-small" uk-grid>
                        <li v-if="pkg.demo">
                          <a class="uk-button uk-button-default" :href="pkg.demo" target="_blank">{{
                            'Demo' | trans
                          }}</a>
                        </li>
                        <li v-if="pkg.support">
...
export default {
  ...
  methods: {
    query() {
      ...
      this.$http.post(url, { q: this.search, type: this.type, page: this.page }, options).then(
        res => {
          const { data } = res;
    ...
    marked(text) {
      const correctHeadings = function (str) {
        const headings = / *(#{1,6}) *([^\n]+?) *(?:#+ *)?(?:\n+|$)/g;
        [...str.matchAll(headings)].length &&
          [...str.matchAll(headings)].forEach(arr => {
