<template>
  <div
    v-if="!isSource"
    class="uk-panel uk-placeholder uk-placeholder-large uk-text-center uk-visible-toggle"
  >
    <img
      width="60"
      height="60"
      :alt="'Placeholder Image' | trans"
      :src="$url('app/system/assets/images/placeholder-image.svg')"
    />
  ...
  <div v-else class="uk-inline-clip uk-transition-toggle uk-visible-toggle">
    <img
      v-if="hasAttribute('uk-img')"
      :data-src="$url(image.data['data-src'])"
      :src="$url(image.data.src)"
      :alt="image.data.alt"
      :uk-img="image.data['uk-img'] ? $url(image.data['uk-img']) : ''"
    />
    <img
      v-else-if="hasAttribute('uk-svg')"
      :data-src="$url(image.data['data-src'])"
      :src="$url(image.data.src)"
      :alt="image.data.alt"
      :uk-svg="image.data['uk-svg'] ? $url(image.data['uk-svg']) : ''"
    />
