  ...
  <v-modal ref="modal" :closed="close">
    <form class="uk-form-stacked" @submit.prevent="update">
      <div class="uk-modal-header">
        <h2 class="uk-h4">
          <span uk-icon="video" ratio="1.1" /><span class="uk-text-middle uk-margin-small-left">{{
            'Video' | trans
          }}</span>
        </h2>
      </div>
      ...
      <div class="uk-modal-body">
          ...
          <div>
            <div class="uk-child-width-1-2 uk-grid-small" uk-grid>
              <div>
                ...
                <input
                  id="form-width"
                  v-model="video.data.width"
                  class="uk-width-1-1 uk-input"
                  type="text"
                  :placeholder="'auto' | trans"
                />
              </div>
              <div>
                ...
                <input
                  id="form-height"
                  v-model="video.data.height"
                  class="uk-width-1-1 uk-input"
                  type="text"
                  :disabled="!isVimeo && !isYoutube"
                  :placeholder="'auto' | trans"
                />
              </div>
            </div>
          ...
          <div>
            <div class="uk-child-width-1-2 uk-grid-small uk-text-small" uk-grid>
              <div class="uk-text-nowrap uk-text-truncate">
                <label
                  ><input v-model="video.data.autoplay" class="uk-checkbox" type="checkbox" />
                  {{ 'Autoplay' | trans }}</label
                ><br />
                <label v-show="!isVimeo"
                  ><input v-model="video.data.controls" class="uk-checkbox" type="checkbox" />
                  {{ 'Controls' | trans }}</label
                >
              </div>
              <div class="uk-text-nowrap uk-text-truncate">
                <label
                  ><input v-model="video.data.loop" class="uk-checkbox" type="checkbox" />
                  {{ 'Loop' | trans }}</label
                ><br />
                <label v-show="!isVimeo && !isYoutube"
                  ><input v-model="video.data.muted" class="uk-checkbox" type="checkbox" />
                  {{ 'Muted' | trans }}</label
                >
              </div>
            </div>
        ...
        <div v-show="!isYoutube && !isVimeo" class="uk-margin">
          ...
          <div class="uk-form-controls">
            <input-image
              v-model="video.data.poster"
              :input-field="false"
              class-name="uk-width-1-1"
            />
          </div>
        </div>
      </div>
  ...
  computed: {
    isYoutube() {
      return this.video.data.src
        ? this.video.data.src.match(/.*(?:youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=)([^#\&\?]*).*/)
        : false;
    },

    isVimeo() {
      return this.video.data.src
        ? this.video.data.src.match(
