...
import { $, data, append, empty, trigger, closest } from 'uikit-util';
...
UIkit.component('Pagination', {
  ...
  methods: {
      ...
      return {
        start: Math.ceil(
          this.currentPage > this.halfDisplayed
            ? Math.max(
                Math.min(this.currentPage - this.halfDisplayed, this.pages - this.displayedPages),
                0
              )
            : 0
        ),
          ...
          this.currentPage > this.halfDisplayed
            ...
            : Math.min(this.displayedPages, this.pages)
        )
      };
    },
    ...
    _append(pageIndex, opts) {
      ...
      options = { text: pageIndex + 1, ...opts };

      item =
        pageIndex == this.currentPage
          ? `<li class="uk-active"><span>${options.text}</span></li>`
          : `<li><a href="#page-${pageIndex + 1}" data-page="${pageIndex}">${options.text}</a></li>`;

      if (typeof options.text === 'string') {
        const str = String(options.text);
        item =
          pageIndex == this.currentPage
            ? `<li class="uk-hidden"><span>${options.text}</span></li>`
            : item;
      }
