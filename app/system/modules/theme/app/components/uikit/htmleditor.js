...
UIkit.component('Htmleditor', {
  ...
  methods: {
    create() {
      Promise.all([this.init(), this.initBase(), this.initMarked(), this.debouncedRedraw()]).then(
        () => {
          setTimeout(() => {
    ...
    init() {
      ...
      if (this.iframe) {
        ...
        if (typeof this.iframe === 'string') {
          append(
            this.preview.container.parentElement,
            `<link rel="stylesheet" href="${this.iframe}">`
          );
        }
      } else {
      ...
      on(
        ...
        debounce(() => {
          ...
          const previewHeight =
            ...
            ($this.iframe ? height($this.$iframe) : height(previewContainer));
          ...
          if ($this.htmleditor.getAttribute('data-mode') == 'tab') {
            ...
            addClass(
              findAll(
                '.uk-htmleditor-button-code, .uk-htmleditor-button-preview',
                $this.htmleditor
              ).filter(el => el === e.target.parentNode),
              'uk-active'
            );

            $this.activetab = hasClass($(e.target.parentNode), 'uk-htmleditor-button-code')
              ? 'code'
              : 'preview';
      ...
      if (
        ...
        this.editor.on(
          ...
          debounce(() => {
            ...
            const mode = $this.CodeMirror.innerMode(
              $this.editor.getMode(),
    ...
    _buildtoolbar() {
      ...
      this.toolbar.forEach(button => {
        ...
        bar.push(
          `<li><a data-htmleditor-button="${button}" title="${title}" data-uk-tooltip>${$this.buttons[button].label}</a></li>`
        );
      });
    ...
    fit() {
      ...
      if (mode == 'tab') {
        ...
        removeClass(
          findAll('.uk-htmleditor-button-code, .uk-htmleditor-button-preview', this.htmleditor),
          'uk-active'
        );
        addClass(
          findAll(
      ...
      css(
        ...
        'height',
        height(this.code) || css($('.CodeMirror', this.code), 'height')
      );
    ...
    initBase() {
      ...
      const listfn = function (tag) {
        if (editor.getCursorMode() == 'html') {
          ...
          if (!inList) {
            ...
            cm.replaceRange(
              `${cm.getLine(posend.line + 1)}\n` + `</${tag}>`,
              { line: posend.line + 1, ch: 0 },
      ...
      on(editor.htmleditor, 'click', 'a[data-htmleditor-button="fullscreen"]', e => {
        ...
        if (hasClass(editor.htmleditor, 'uk-htmleditor-fullscreen')) {
          ...
          if (fixedParent) {
            fixedParent = $parents.filter(
              el =>
                fixedParent.tagName !== 'html' &&
                fixedParent.parentNode.outerHTML.indexOf(el.outerHTML) !== -1 &&
                fixedParent.parentNode !== el
            );
    ...
    initMarked() {
      ...
      on(editor.$el, 'enableMarkdown', () => {
        editor.enableMarkdown();
      });
      on(editor.$el, 'disableMarkdown', () => {
