...
Vue.ready({
    ...
    elements() {
      ...
      return {
        ...
        addpage: {
          ...
          items: () =>
            vm.orderBy(vm.protected(vm.types), 'label').map(type => {
              const props = {
                attrs: {
                  href: () =>
                    vm.$url.route('admin/site/page/edit', { id: type.id, menu: vm.menu.id })
                },
                caption: () => type.label
              };
              return { ...type, ...props };
  ...
  created() {
    ...
    this.$watch(
      _vm => (_vm.menu, _vm.nodes, Date.now()),
      () => {
        this.tree('update');
      },
      { deep: true }
  ...
  methods: {
        ...
        flatten() {
          ...
          const flatten = (children, extractChildren, level, order) =>
            Array.prototype.concat.apply(
              ...
              children.map(x =>
                flatten(extractChildren(x) || [], extractChildren, (level || 1) + 1)
              )
            );

          const extractChildren = x => x.children;

          const flat = flatten(extractChildren(treeStructure), extractChildren).map(
    ...
    isSelected(node, children) {
      ...
      return (
        this.selected.indexOf(node.id) !== -1 &&
        (!children || !this.tree[node.id] || this.isSelected(this.tree[node.id], true))
      );
    },
    ...
    divided(menus) {
      return _.reject(menus, { fixed: true }).concat(
        { divider: true },
        _.filter(menus, { fixed: true })
      );
    },
  ...
  computed: {
    showDelete() {
      ...
      return (
        this.showMove &&
        _.every(this.getSelected(), node => !(vm.getType(node) || {}).protected, this)
      );
    },
