<template>
  <div>
      ...
      <form class="uk-form-stacked">
        ...
        <div class="uk-margin">
          ...
          <div class="uk-form-controls uk-form-controls-text">
            <p class="uk-margin-small">
              <label
                ><input v-model="widget.total" class="uk-radio" type="radio" value="1" />
                {{ 'Show' | trans }}</label
              >
            </p>
            <p class="uk-margin-small">
              <label
                ><input v-model="widget.total" class="uk-radio" type="radio" value="" />
                {{ 'Hide' | trans }}</label
              >
            </p>
          </div>
    ...
    <div class="uk-card-body">
      ...
      <h3
        v-if="widget.show == 'registered' && widget.total"
        class="uk-card-title uk-margin-small-top"
      >
        {{
          '{0} Registered Users|{1} Registered User|]1,Inf[ Registered Users'
            | transChoice(userscount)
        }}
      </h3>
      ...
      <ul
        v-if="users.length && widget.display == 'thumbnail'"
        data-user
        class="uk-grid-small uk-child-width-1-5 uk-child-width-1-5@m uk-child-width-1-6@xl"
        uk-grid
      >
        <li v-for="user in users" :key="user.id">
          <a :href="$url.route('admin/user/edit', { id: user.id })" :title="user.username">
            <img
              v-gravatar="user.email"
      ...
      <ul v-if="users.length && widget.display == 'list'" data-user class="uk-list uk-list-divider">
        <li v-for="user in users" :key="user.id" class="uk-flex uk-flex-middle">
          ...
          <div class="uk-flex-1 uk-text-truncate">
            <a :href="$url.route('admin/user/edit', { id: user.id })" :title="user.name">{{
              user.username
            }}</a>
