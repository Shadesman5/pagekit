<template>
  <div uk-grid class="">
    <div class="uk-width-3-4@m uk-width-2-3">
      ...
      <div class="uk-margin">
        ...
        <div v-show="user.id && !editingPassword" class="uk-form-controls uk-form-controls-text">
          <a href="#" class="uk-text-small" @click.prevent="editingPassword = true">{{
          ...
          <p v-for="(status, key) in config.statuses" :key="key" class="uk-margin-small">
            <label
              ><input
                ...
                type="radio"
                :value="parseInt(key)"
                :disabled="config.currentUser == user.id"
              />
              {{ status }}</label
            >
          </p>
    ...
    <div v-show="user.email" class="uk-width-1-4@m uk-width-1-3">
      <div class="uk-card uk-card-default uk-text-center uk-text-truncate">
        ...
        <div class="uk-card-footer uk-padding-small">
          ...
          <a
            class="uk-flex uk-flex-center uk-flex-middle uk-text-small"
            :href="'mailto:' + user.email"
          >
            <span class="uk-text-truncate">{{ user.email }}</span>
            <i
              v-show="config.emailVerification && user.data.verified"
              class="uk-preserve-width uk-text-success"
              uk-icon="icon: check"
              ratio="1.1"
              :title="'Verified email address' | trans"
            />
          </a>
        </div>
      </div>
