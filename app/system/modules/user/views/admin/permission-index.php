<?php declare(strict_types=1); $view->script('permission-index', 'system/user:app/bundle/permission-index.js', 'vue') ?>

<div id="permissions" v-cloak>

    <h2 class="uk-h3">{{ 'Permissions' | trans }}</h2>

    <div :id="key_group" class="uk-overflow-auto uk-margin" v-for="(group, key_group) in permissions" :key="key_group">
        <table class="uk-table uk-table-hover uk-table-middle">
            <thead>
                <tr>
                    <th class="pk-table-min-width-200">{{ key_group }}</th>
                    <th class="uk-table-shrink"></th>
                    <th class="uk-table-shrink pk-table-width-100 pk-table-max-width-100 uk-text-truncate uk-text-center" v-for="(r, key_r) in roles" :key="key_r">{{ r.name }}</th>
                </tr>
            </thead>
            <tbody>
