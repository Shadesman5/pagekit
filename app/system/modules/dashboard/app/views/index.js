...
window.Dashboard = {
  ...
  created() {
    ...
    this.$set(
      this,
      'widgets',
      this.widgets.filter((widget, idx) => {
  ...
  methods: {
    ...
    getColumn(column) {
      ...
      return _.sortBy(
        this.widgets.filter(widget => widget.column === column),
        'idx'
      );
    },
    ...
    add(type) {
      ...
      this.Widgets.save({
        widget: _.merge({ type: type.id, column, idx: 100 }, type.defaults)
      }).then(function (res) {
        const { data } = res;
