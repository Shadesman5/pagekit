        ...
        <div class="uk-margin">
          ...
          <div class="uk-form-controls uk-form-controls-text">
            <p class="uk-margin-small">
              <label
                ><input v-model="widget.content" class="uk-radio" type="radio" value="" />
                {{ "Don't show" | trans }}</label
              >
            </p>
            <p class="uk-margin-small">
              <label
                ><input v-model="widget.content" class="uk-radio" type="radio" value="1" />
                {{ 'Show on all posts' | trans }}</label
              >
            </p>
            <p class="uk-margin-small">
              <label
                ><input v-model="widget.content" class="uk-radio" type="radio" value="2" />
                {{ 'Only show on first post.' | trans }}</label
              >
            </p>
          </div>
        </div>
    ...
    <div class="uk-card-body">
      <div v-if="status != 'loading'">
        ...
        <div v-else class="uk-margin-bottom">
          ...
          <p
            v-if="!widget.title && head.title && head.description"
            class="uk-margin-remove uk-text-small uk-text-meta"
          >
            {{ head.description }}
          </p>
        </div>
        ...
        <ul class="uk-list uk-list-divider uk-margin-remove">
          <li v-for="(entry, key) in count" :key="key">
            ...
            <p
              class="uk-text-small uk-text-muted uk-text-nowrap uk-margin-remove-top uk-margin-small-bottom"
            >
              {{ entry.publishedDate | relativeDate }}
            </p>

            <div
              v-if="widget.content == '1'"
              class="uk-margin-small-top uk-text-justify uk-text-small"
              v-html="entry.contentSnippet"
            />

            <div
              v-if="widget.content == '2'"
              class="uk-margin-small-top uk-text-justify uk-text-small"
              v-html="key == 0 ? entry.contentSnippet : ''"
            />
          </li>
        </ul>
...
export default {
  ...
  methods: {
    load() {
      ...
      this.$http
        .get(`https://api.rss2json.com/v1/api.json?rss_url=${this.widget.url}`, {
          params: { cache: 60 }
        })
        .then(
          function (res) {
