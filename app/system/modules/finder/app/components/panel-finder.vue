  ...
  <div v-show="items" class="storage">
    <template v-if="!modal">
          ...
          <h2 class="uk-h3 uk-margin-remove">
            ...
            <span v-else>{{
              '{1} %count% File selected|]1,Inf[ %count% Files selected'
                | transChoice(selected.length, { count: selected.length })
            }}</span>
          </h2>
      ...
      <div class="uk-flex uk-flex-middle">
        ...
        <span class="uk-text-small uk-text-bolder" style="margin-right: 2px"
          >{{ 'Folder' | trans }}:</span
        >
      ...
      <div
        class="uk-flex uk-overflow-auto tm-overflow-container"
        ...
        <div class="uk-position-relative uk-flex uk-flex-between">
          <div class="uk-flex uk-flex-middle uk-flex-nowrap">
            <h1 class="uk-h4 uk-flex uk-flex-middle uk-flex-nowrap uk-margin-remove-bottom">
              <span
                class="uk-visible@s uk-margin-small-right"
                uk-icon="database"
                ratio="1.1"
              /><span>{{ 'Storage' | trans }}</span>
            </h1>
            <div v-if="isWritable && selected.length" class="uk-margin-left">
...
const PanelFinder = {
  ...
  computed: {
    breadcrumbs() {
      const path = '';
      const crumbs = [{ path: '/', title: this.getRoot() }].concat(
        this.path
          .substr(1)
          .split('/')
    ...
    searched() {
      ...
      return _.filter(
        this.items,
        file => !vm.search || file.name.toLowerCase().indexOf(vm.search.toLowerCase()) !== -1
      );
    },
  ...
  methods: {
    ...
    command(cmd, params) {
      return this.resource
        .save({ cmd }, _.extend({ path: this.path, root: this.getRoot() }, params))
        .then(
          function (res) {
            this.load();
    ...
    initUpload() {
      ...
      UIkit.upload(
        UIkit.util.parents(this.$el, '.uk-modal').length ? this.$el : UIkit.util.$('html'),
        settings
      );
    }
  },
