...
function renderElement(h, element, _vue) {
  ...
  const render = {
          ...
          [
            h('ul', { class: 'uk-nav uk-dropdown-nav' }, [
              $this.orderBy(get(this, 'items'), 'priority').map((item, name) => {
                ...
                return (
                  get(item, 'vif') &&
                  h('li', { class: { 'has-dropdown': item.actionDropdown } }, [
    ...
    button() {
      ...
      const elements = h => {
        ...
        spinner && elms.push([render.spinner(h)]);
        icon &&
          internal &&
          elms.push([
            h('span', {
        ...
        if (get(this, 'caption')) {
          if ((icon || spinner) && !internal) {
            elms.push([h('span', { class: 'uk-text-middle' }, getText(get(this, 'caption')))]);
          } else if (icon && internal) {
            elms.push([h('span', { class: 'uk-text-middle' }, getText(get(this, 'caption')))]);
          } else if (!icon) {
            elms.push([getText(get(this, 'caption'))]);
          }
        }
        return elms;
      };
      ...
      return h(
        'a',
        {
          attrs: internal
            ? get(this, 'attrs')
            : _.merge({}, get(this, 'attrs'), get(this, 'icon.attrs')),
    ...
    iconnav() {
      ...
      return h(
        ...
        get(this, 'items').map(
          item =>
            get(item, 'vif') &&
            h('li', { class: { 'uk-active': get(item, 'active') } }, [create(h, item)])
        )
      );
    },
    caption() {
  ...
  return element.type && typeof render[element.type] === 'function'
    ? render[element.type].call(element)
    : '';
}
  ...
  const render = {
      ...
      h(
        ...
        [
          h('div', { class: ['tm-topmenu-item'] }, [
            h(
              'ul',
              { class: 'tm-action-buttons uk-visible@m' },
              this.orderBy(items, 'priority').map(
                element =>
                  !isElementEmpty(element) &&
                  get(element, 'vif') &&
                  h('li', { class: { 'uk-disabled': get(element, 'disabled') } }, [
    ...
    sidemenu: () => {
      ...
      return h(
        'ul',
        {
          class: [
            'tm-action-buttons',
            !sb ? 'uk-nav uk-nav-default' : !breakpoint ? 'uk-nav uk-nav-default' : 'uk-iconnav',
            { 'uk-flex-nowrap': sb && isEmpty }
          ]
        },
        [
          h('li', { class: ['tm-toggle-sidebar', { 'uk-flex-last': sb }] }, [
            h(
              'a',
              {
                ...
                attrs: {
                  'uk-tooltip': !sb ? this.$trans('Show') : false,
                  pos: !sb ? 'right' : false
                }
              },
              [
                h('span', {
                  attrs: { 'uk-icon': !sb ? 'more' : 'more-vertical' },
                  class: 'tm-menu-image'
                })
              ]
            )
            ...
            this.orderBy(items, 'priority').map(
              (element, id) =>
                ...
                h(
                  'li',
                  {
                    class: [{ 'uk-disabled': get(element, 'disabled') }, { 'uk-flex-first': !id }]
                  },
                  [renderElement(h, element, this)]
                )
            ),
          sb &&
            h('li', { class: 'uk-width-expand' }, [
              isEmpty &&
                h(
                  'div',
                  {
                    class: [
                      'uk-height-1-1 uk-flex uk-flex-middle uk-light',
                      { 'uk-hidden': !breakpoint }
                    ]
                  },
                  [h('span', this.$trans('Extensions'))]
                )
            ])
        ]
      );
    ...
    navbar: () =>
      h(
        ...
        this.orderBy(items, 'priority').map(
          element =>
            !isElementEmpty(element) && get(element, 'vif') && renderElement(h, element, this)
        )
      )
  };
...
const ThemeTopMenu = {
  render(h) {
    ...
    const isScopes = scopes
      .map(scope => $theme.isScopeEmpty($theme.getScope(scope)))
...
ThemeHelper.prototype._initEvents = function () {
  this.theme.$on('theme-menu-created', () =>
    this.createElements().then(() => this.theme.$emit('theme-menu-items-ready'))
  );
  ...
  elements &&
    ...
    _.forEach(elements, (element, name) => {
      ...
      if (isObject(element) && isElement(element.type) && !this.isProtected(path)) {
        ...
        (!isObject(element) || !isElement(element.type)) &&
          !this.isProtected(path) &&
          this.log('warn', 'empty', path);
      }
    });
};
...
Object.defineProperties(ThemeHelper.prototype, {
  ...
  elementsOption: {
    get() {
      return this.option && isFunction(this.option.elements)
        ? this.option.elements.bind(this.$vm).call()
        : null;
    }
  },
  extendOption: {
