...
export default Vue => {
  Vue.filter('baseUrl', url =>
    _.startsWith(url, Vue.url.options.root) ? url.substr(Vue.url.options.root.length) : url
  );
  Vue.filter('trans', (id, parameters, domain, locale, replace) =>
    Vue.prototype.$trans(id, parameters, domain, locale, replace)
  );
  Vue.filter('transChoice', (id, number, parameters, domain, locale) =>
    Vue.prototype.$transChoice(id, number, parameters, domain, locale)
  );
  Vue.filter('trim', {
    write(value) {
      return value.trim();
  ...
  Vue.filter('currency', (amount, currencySymbol, fractionSize) =>
    Vue.prototype.$currency(amount, currencySymbol, fractionSize)
  );
