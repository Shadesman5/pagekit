...
export default {
  ...
  update(el, binding, vnode) {
    ...
    binding.keypath = binding.rawName.indexOf(binding.arg)
      ? binding.rawName.slice(binding.rawName.indexOf(binding.arg))
      : binding.arg;
    ...
    binding.handler = [
      ...
      function (e) {
        e.stopImmediatePropagation();
        if (
          !(util.isInput(e.target) || e.target.tagName === 'A') &&
    ...
    binding.unbindWatcher = vnode.context.$watch('selected', selected => {
      util.findAll(binding.selector, vnode.context.$el).forEach(elem => {
        elem.checked =
          selected.indexOf(binding.def.toNumber(elem.value, el, binding, vnode)) !== -1;
      });
