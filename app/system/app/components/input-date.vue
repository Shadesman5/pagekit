  ...
  <div class="uk-grid-small uk-child-width-1-2@l" uk-grid>
    <div>
      <div ref="datepicker" class="uk-inline">
        ...
        <input v-model.lazy="date" class="uk-input" type="text" />
      </div>
    </div>
    <div>
      <div ref="timepicker" class="uk-inline">
        <span class="uk-form-icon" uk-icon="clock" />
        <input v-model.lazy="time" class="uk-input" type="text" />
      </div>
    </div>
  </div>
...
const InputDate = {
  ...
  computed: {
    ...
    date: {
      get() {
        return flatpickr.formatDate(new Date(this.datetime), 'Y-m-d\\Z').slice(0, -1);
      },
    ...
    time: {
      ...
      set(time) {
        ...
        const hour = parseInt(time[0]) + (fulltime.indexOf('PM') !== -1 ? 12 : 0);
...
Vue.component('InputDate', (resolve, reject) => {
  Vue.asset({
