<template>
  <div>
    <a
      v-if="!source"
      class="uk-placeholder uk-display-block uk-text-muted uk-text-center uk-margin-remove"
      @click.prevent="pick"
    >
    ...
    <v-modal ref="modal" large>
      ...
      <div class="uk-modal-footer">
        <div class="uk-flex uk-flex-middle uk-flex-between">
          <div>
            <div v-if="isFinder">
              <span v-if="!finder.selected.length" class="uk-text-meta">{{
                '{0} %count% Files|{1} %count% File|]1,Inf[ %count% Files'
                  | transChoice(finder.count, { count: finder.count })
              }}</span>
              <span v-else class="uk-text-meta">{{
                '{1} %count% File selected|]1,Inf[ %count% Files selected'
                  | transChoice(finder.selected.length, { count: finder.selected.length })
              }}</span>
            </div>
          </div>
          <div>
  ...
  methods: {
    ...
    update(src) {
      ...
      if (matches) {
        ...
        this.$http
          .get('http://vimeo.com/api/oembed.json', { params: { url: src }, cache: 10 })
          .then(res => {
            const { data } = res;
