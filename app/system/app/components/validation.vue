<template>
  <ValidationObserver slim>
    <ValidationProvider v-slot="{ errors }" :name="$attrs.name" :rules="rules" slim>
      <div
        v-if="!get('type')"
        :class="get('containerClass') ? get('containerClass') : 'uk-form-controls'"
      >
        <input
          v-if="!get('tag')"
          :id="$attrs.id"
          ...
          :name="$attrs.name"
          :type="$attrs.type"
          :placeholder="$attrs.placeholder | trans"
          :class="[get('class'), { 'uk-form-danger': get('danger') && errors[0] }]"
          :autocomplete="$attrs.autocomplete"
        />
        <textarea
          v-else-if="get('tag') === 'textarea'"
          :id="$attrs.id"
          v-model="innerValue"
          :name="$attrs.name"
          :type="$attrs.type"
      ...
      <div v-else-if="get('type') === 'icon'" :class="get('containerClass')">
        <div class="uk-inline">
          <a
            v-if="get('iconTag') === 'a'"
            tabindex="-1"
            :class="['uk-form-icon', { 'uk-form-icon-flip': get('iconDir') === 'right' }]"
            href="#"
            :style="get('iconLabel') ? { width: 'auto', marginRight: '10px' } : ''"
            @click.prevent="get('iconClick')"
          >
            <span
              v-if="get('iconDir') === 'right' && get('iconLabel')"
              class="uk-margin-small-right uk-text-small"
              >{{ get('iconLabel') | trans }}</span
            >
            <span :uk-icon="get('icon')" />
          </a>
          <span
            v-else
            :class="['uk-form-icon', { 'uk-form-icon-flip': get('dir') === 'right' }]"
            href="#"
            :uk-icon="get('icon')"
          />
          <input
