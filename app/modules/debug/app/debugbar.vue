<template>
  <div id="pk-profiler" class="pf-profiler">
    <div class="pf-navbar">
      <ul v-if="data" class="pf-navbar-nav">
        <li v-for="section in sections" :key="section.name">
          <component
            :is="section.name"
            :data="data[section.name == 'TimeComponent' ? 'Time' : section.name]"
            @click.native="open(section.name)"
          />
        </li>
      </ul>
    ...
    <div
      ref="panel"
      class="pf-profiler-panel"
      :style="{ display: panel ? 'block' : 'none', height: height }"
    />
  </div>
</template>
  ...
  methods: {
    load(id) {
      return this.$http.get('_debugbar/{id}', { params: { id } }).then(res => {
        const entries = Object.entries(res.data).map(entry => [
