<template>
  <v-modal ref="output" :options="options">
    <div class="uk-modal-header uk-flex uk-flex-middle">
      <h2 class="uk-h4">
        {{
          'Updating %title% to %version%' | trans({ title: pkg.title, version: updatePkg.version })
        }}
      </h2>
    </div>
    ...
    <div class="uk-modal-body">
      ...
      <div v-show="status == 'loading'" class="uk-alert uk-flex uk-flex-middle uk-background-muted">
        ...
        <span v-show="showOutput" class="uk-margin-small-left"
          >{{
            'Updating %title% to %version%'
              | trans({ title: pkg.title, version: updatePkg.version })
          }}...</span
        >
      </div>
      ...
      return this.$http
        ...
        .then(
          () => {
            ...
            if (pkg.enabled) {
              this.$parent.enable(pkg).then(null, () => {
                this.status = 'error';
              });
            }
          },
