...
if (TASK_PROMPT.split('/').includes('..'))
  fail(`TASK_PROMPT must not contain '..' path segments: ${TASK_PROMPT}`);
...
if (!Number.isInteger(RESULT_GRACE_MS) || RESULT_GRACE_MS < 0)
  fail(`Invalid RESULT_GRACE_MS: ${process.env.RESULT_GRACE_MS}`);
...
(async () => {
  ...
  if (!AUDIT && !ISSUE)
    fail(
      'ISSUE is required for non-audit runs (PR Closes #N + conductor:stop/pause labels). Only audit tasks (a `conductor-mode: plan` marker in the task prompt) may omit it.'
    );
  ...
  if (!alreadyArchived) {
    ...
    if (MODE === 'plan') {
      const detail = AUDIT
        ? planRan
          ? ' (audit: report + PR opened this run)'
          : ' (audit: PR already open)'
        : '';
      metrics.setSessionStatus('completed');
      log(
        `✅ Conductor done (mode=plan): Step-0 gate complete${detail} - EXECUTE/FINALIZE skipped by design.`
      );
      return;
    }
    ...
    if (steps === null)
      fail(
        `ticket not found after PLAN: ${TICKET} (audit/report task? re-dispatch with MODE=plan for a Step-0-gate-only run)`
      );
    if (steps.length === 0)
      fail(
  ...
  try {
    metrics.setSessionStatus('failed');
  } catch {
    /* best-effort */
  }
...
async function runPhase(label, prompt, outcomeHint) {
  ...
  if (!agentId || !runId)
    throw new Error(`unexpected create response: ${JSON.stringify(created).slice(0, 300)}`);
  ...
  for (;;) {
    ...
    try {
      ...
      if (++fails > MAX_POLL_FAILS)
        throw new Error(`poll failed ${fails}x (last: ${e.message})`, { cause: e });
...
function resolveRun() {
  ...
  if (audit && MODE_INPUT === 'full')
    log(`note: audit task is plan-only; ignoring mode=full (no executable ticket)`);
  return { mode, audit };
}
function readSteps() {
  ...
  for (const line of section.split('\n')) {
    ...
    if (seen.has(n))
      fail(`duplicate Step ${n} in ${TICKET} EXECUTION STATE — step numbers must be unique`);
    seen.add(n);
...
async function gate() {
  for (;;) {
    ...
    if (s === 'stop') {
      ...
      try {
        metrics.setSessionStatus('cancelled');
      } catch {
        /* best-effort */
      }
      process.exit(3);
    ...
    if (s === 'pause') {
      log('⏸ conductor:pause — waiting (remove the label to resume)…');
      await sleep(30000);
      continue;
    }
    return;
...
async function logUsage(agentId) {
  try {
    ...
    log(
      `  tokens: in=${t.inputTokens ?? '?'} out=${t.outputTokens ?? '?'} cacheR=${t.cacheReadTokens ?? '?'} cacheW=${t.cacheWriteTokens ?? '?'} total=${t.totalTokens ?? '?'}`
    );
  } catch {
    /* best-effort */
  }
...
function donePrExists() {
  try {
    return (
      Number(
        sh(
          `gh pr list --head ${BRANCH} --base ${BASE} --state all --json state --jq '[.[] | select(.state=="OPEN" or .state=="MERGED")] | length'`
        )
      ) > 0
    );
  } catch (e) {
...
function validate(name, val, re) {
  if (val && !re.test(val)) {
    console.error(`Invalid ${name}: ${val}`);
    process.exit(1);
  }
}
