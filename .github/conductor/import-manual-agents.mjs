...
async function main() {
  ...
  console.log(`Import ${agentIds.length} manual agent(s) → step ${stepId}${DRY_RUN ? " [dry-run]" : ""}`);
  ...
  for (const agentId of agentIds) {
    ...
    const phase = {
      ...
      type: label.startsWith("EXECUTE") || label === "PLAN" || label === "FINALIZE" ? label.split(" ")[0] : "MANUAL",
      attempt: 0,
      batchSteps: null,
      agent: {
        id: agentId,
