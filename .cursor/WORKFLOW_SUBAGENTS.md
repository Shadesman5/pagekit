...
**V1 (cursor.com/agents UI):** `.cursor/rules/orchestrator-subagent-workflow.mdc` — same subagents, one session. Full-ticket V1 metrics import after merge (`v1-metrics`). A Conductor XL handoff is imported at FINALIZE (best-effort) — see `.github/conductor/metrics/README.md`.
...
EXECUTE  per batch (Conductor) / all remaining steps (V1)
         ...
         Re-dispatch Conductor with the same `session_id` for FINALIZE (that job
         imports the V1 XL parent, best-effort). (`handoff_xl=false` launches XL as a cloud batch.)
        │
        ▼
