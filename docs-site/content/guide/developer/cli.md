...
Available commands:
 ...
 migration
  ...
  migration:migrate    Execute database migrations
...
Pagekit's database schema is managed by **Doctrine Migrations 3.x**. The `migration:*` commands run, generate and inspect those migration files.
...
```sh
./pagekit migration:generate MigrationName
```

This creates a new migration file in `app/migrations/` (organized by year) that you can edit with your schema changes.
...
```sh
./pagekit migration:rollback [--to=VERSION]
```

**Note** Migrations are executed automatically during Pagekit updates and extension installations. You typically run these commands manually only while developing extensions or troubleshooting. See the [database documentation](database.md#database-migrations) for the migration workflow.
