...
**Note:** Pagekit currently uses Vue.js 2.6 with UIkit 3.5. The frontend build pipeline uses Webpack 4. **Dependency installation** requires Yarn (`yarn install` – npm install is blocked). **Asset compilation** works with either: `yarn compile-js` / `yarn watch-js`, `npm run compile-js` / `npm run watch-js`, or directly `webpack` / `webpack --watch` (via npx or from `node_modules/.bin`).
...
Webpack is a tool that runs on your terminal and then compiles the `*.vue` files to `*.js` files. Pagekit has a default `webpack.config.js` on the root level. To compile assets, run one of the following in the Pagekit folder:

- `yarn compile-js` or `yarn watch-js` (recommended, uses package scripts)
- `npm run compile-js` or `npm run watch-js`
- `webpack` or `webpack --watch` directly (via `npx webpack` or `./node_modules/.bin/webpack`)

This traverses all themes and extensions in the `packages` subfolders.
...
Now open the terminal, navigate to the Pagekit root folder and run `yarn compile-js` (or `yarn watch-js` for development), `npm run compile-js`, or `webpack` directly to compile your assets. You will see `*.js` files appearing in the `app/bundle` folder of your package. For example `example.vue` is compiled into a bundle file called `example.js`.
...
3. Run `yarn compile-js` (or `yarn watch-js`), `npm run compile-js`, or `webpack` in the root directory of your Pagekit installation. The webpack config of Pagekit is setup so that it traverses all packages and reads their webpack configurations as well.
