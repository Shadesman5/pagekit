...
A simple way to then protect a controller action is by using an attribute as follows. Read about [attributes](routing.md#attributes) to find out more.
...
To read the config of a module, fetch the module instance from the `module` service and access its `config` property. This object is the merge of the default config from `index.php` and the values persisted in the database.

```php
$config = $app->get('module')->get('hello')->config;
```

### Write config

To persist module configuration changes, write through the `config` service (`Pagekit\Config\ConfigManager`). The values are flushed to the database automatically.

```php
// Replace the entire config of a module
$app->get('config')->set('hello', $config);

// Update a single value
$app->get('config')->get('hello')->set('message', 'Custom message');
```

**Note** Reading `module->config` directly after a write still returns the previously merged values. On the next request, Pagekit re-merges defaults with the persisted values and exposes the new state via the module's `config` property.
