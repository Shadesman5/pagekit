...
- **GitHub Actions runtime bumped to Node 24** — all eight action pins in `.github/workflows/php-quality.yml` lifted to the current major: `actions/checkout@v4` → `@v5` (4×), `actions/cache@v4` → `@v5` (3×), `actions/upload-artifact@v4` → `@v5` (1×). The v5 majors run on the `node24` runtime (released 2025-08), eliminating the `Node.js 20 actions are deprecated` annotations GitHub started attaching to every CI run ahead of the platform-wide `node20` shutoff. `ubuntu-latest` runners (24.04 LTS) already meet the new minimum runner requirement (≥ v2.327.1). No workflow logic changed — pure runtime hygiene; the four jobs (`phpunit (8.2)`, `phpunit (8.3)`, `phpstan`, `cs-fixer`, `security-audit`) keep their existing keys, caches, and gating semantics.
