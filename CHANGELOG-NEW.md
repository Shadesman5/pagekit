...
- **Committed OpenWeatherMap API key removed, along with the widget that needed one** — the hardcoded key first moved into the environment, and then the dashboard's location widget went with it, so no credential and no third-party destination remains: the CSP `connect-src` drops `api.openweathermap.org` and `maps.googleapis.com`, and the administrator's Google key that the widget sent from the browser is gone with them. The removed key stays readable in prior git history and has to be revoked at the provider.
