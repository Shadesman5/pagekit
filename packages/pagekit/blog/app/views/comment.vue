  ...
  <li :id="'comment-' + comment.id">
    <article
      :class="[
        ...
        { 'uk-comment-primary': comment.special },
        { 'tm-reply-visible': replyVisible && !comment.special }
      ]"
    >
      <header class="uk-comment-header uk-position-relative">
          ...
          <div class="uk-width-expand">
            ...
            <p class="uk-comment-meta uk-margin-remove-top">
              <time :datetime="comment.created">{{
                comment.created | relativeDate({ max: 2592000 })
              }}</time>
        ...
        <div
          v-if="comment.status"
          class="uk-text-small uk-position-top-right uk-position-small uk-hidden-hover"
        >
          <a v-if="showReplyButton && user.canComment" href="#" @click.prevent="replyTo">{{
            'Reply' | trans
          }}</a>
        </div>
