...
window.Post = {
  ...
  mounted() {
    ...
    UIkit.util.on(this.tab.connects, 'show', (e, tab) => {
      ...
      const index = tab.toggles.findIndex((toggle, idx) =>
        toggle.parentNode.classList.contains('uk-active')
      );
          ...
          function (res) {
            ...
            if (!this.post.id) {
              window.history.replaceState(
                {},
                '',
                this.$url.route('admin/blog/post/edit', { id: data.post.id })
              );
            }
