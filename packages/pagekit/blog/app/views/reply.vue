      ...
      <form class="uk-form-stacked" @submit.prevent="passes(save)">
        ...
        <div class="uk-margin">
          ...
          <v-input
            ...
            name="content"
            rows="10"
            view="tag: textarea, class: uk-textarea uk-form-width-large"
            rules="required"
...
export default {
  ...
  methods: {
    save() {
      ...
      this.$resource('api/blog/comment{/id}')
        .save({ id: 0 }, { comment })
        .then(
          function (res) {
            const { data } = res;
            ...
            if (!this.user.skipApproval) {
              this.root.messages.push(
                this.$trans('Thank you! Your comment needs approval before showing up.')
              );
            } else {
              this.root.load().then(() => {
