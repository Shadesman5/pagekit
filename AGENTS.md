...
- **First run requires a Pagekit installation.** If `/workspace/config.php` does not exist, the app redirects to the web installer at `/`. For headless/agent setups, prefer the non-interactive CLI: `php pagekit setup -u admin -p '<password>' -t "Pagekit Dev" -m admin@example.com -d sqlite --no-interaction`. Either path creates `config.php` and the SQLite database at `/workspace/pagekit.db`. (Note: the `setup` command prints `Done` and works even though it currently returns a non-zero exit code — verify by checking that `config.php` was created.)
- **Public frontend homepage currently 500s on a fresh install** (`develop` branch). `MetaHelper::add()` is strictly typed `string|array` but `app/system/modules/site/index.php` passes `null` for unset `meta.*` site config (e.g. `meta.twitter`). This is a known pre-existing code bug, NOT an environment/setup problem — the admin backend (`/index.php/admin/login`) and API work fine. Do not treat the homepage 500 as a broken setup.
