...
test.describe('Pagekit System Settings', () => {
  ...
  test('Navigate to System Settings', async ({ page }) => {
    console.log('📍 Testing System Settings navigation...');
  ...
  test('General Settings', async ({ page }) => {
    ...
    // Verify save message
    const successMessage = await page.locator('.uk-alert-success').isVisible().catch(() => false);
  ...
  test('Maintenance Mode', async ({ page }) => {
    ...
    const maintenanceTab = page.locator('a:has-text("Maintenance"), li:has-text("Maintenance")').first();
    ...
    if (await maintenanceToggle.count() > 0) {
      ...
      const maintenanceVisible = await frontendPage.locator('text=/maintenance/i').isVisible().catch(() => false);
  ...
  test('Mail Settings', async ({ page }) => {
    ...
    // From Email
    const fromEmailInput = page.locator('input[name*="from_address"], input[name*="mail"][name*="from"]').first();
    ...
    if (await driverSelect.count() > 0) {
      ...
      // Test email button
      const testButton = page.locator('button:has-text("Send Test Email"), button:has-text("Test")').first();
  ...
  test('Code Editor Settings', async ({ page }) => {
    ...
    if (await editorTab.isVisible()) {
      ...
      // Line numbers
      const lineNumbersCheckbox = page.locator('input[type="checkbox"][name*="line_numbers"]').first();
  ...
  test('System Information', async ({ page }) => {
    ...
    const permissionsTable = page.locator('table').filter({ has: page.locator('text=/writable|readable/i') });
  ...
  test('Cache Management', async ({ page }) => {
    ...
    if (await systemMenu.isVisible()) {
      ...
      if (await cacheLink.isVisible()) {
        ...
        // Clear cache button
        const clearCacheButton = page.locator('button:has-text("Clear Cache"), button:has-text("Clear")').first();
  ...
  test('Update Check', async ({ page }) => {
    ...
    if (await updateLink.isVisible()) {
      ...
      if (await checkUpdateButton.isVisible()) {
        ...
        const upToDate = await page.locator('text=/up to date|no updates/i').isVisible().catch(() => false);
        const updatesAvailable = await page.locator('text=/update available|new version/i').isVisible().catch(() => false);
