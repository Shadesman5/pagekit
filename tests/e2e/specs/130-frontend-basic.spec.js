...
test.describe('Pagekit Frontend (Installed)', () => {
  ...
  test('Navigation menu exists', async ({ page }) => {
    ...
    const navMenu = await page.locator('.uk-navbar-nav').isVisible().catch(() => false);
    const mobileMenu = await page.locator('[data-uk-offcanvas]').isVisible().catch(() => false);
  ...
  test('Footer exists', async ({ page }) => {
    ...
    const copyright = await page.locator('text=/powered by/i').isVisible().catch(() => false);
  ...
  test('404 page handles non-existent URLs', async ({ page }) => {
    ...
    const errorMessage = await page.locator('text=/not found/i').isVisible().catch(() => false);
