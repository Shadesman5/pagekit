...
test.describe('Pagekit Blog Module', () => {
  ...
  test('Navigate to Blog module', async ({ page }) => {
    ...
    if (await blogMenu.isVisible()) {
      ...
      const addPost = await page.locator('a:has-text("Add Post"), button:has-text("Add Post")').isVisible();
  ...
  test('Create a new blog post', async ({ page }) => {
    ...
    if (await addButton.isVisible()) {
      ...
      if (await contentFrame.count() > 0) {
        await contentFrame.locator('body').fill('This is a test blog post created by E2E tests. It contains some sample content to verify the blog functionality.');
      } else {
  ...
  test('Edit existing blog post', async ({ page }) => {
    ...
    if (await postRow.count() > 0) {
      ...
      const contentFrame = page.frameLocator('iframe.uk-htmleditor-iframe').first();
      if (await contentFrame.count() > 0) {
        await contentFrame.locator('body').fill('Updated content for the test blog post. This has been modified by E2E tests.');
      }
