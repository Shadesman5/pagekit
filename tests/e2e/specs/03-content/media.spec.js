...
test.describe.fixme('Pagekit Media Management (Finder)', () => {
  ...
  test('Navigate to Storage/Media manager', async ({ page }) => {
    ...
    const storageLink = page
      .locator('a[href*="/storage"], a:has-text("Storage"), a:has-text("Media")')
      .first();
    ...
    if (await storageLink.isVisible()) {
      ...
      const folderButton = await page
        .locator('button:has-text("Folder"), a:has-text("Add Folder")')
        .isVisible();
  ...
  test('Create folders', async ({ page }) => {
    ...
    const folderButton = page
      .locator('button:has-text("Folder"), button:has-text("Add Folder")')
      .first();
    if (await folderButton.isVisible()) {
      ...
      if (await folderInput.isVisible()) {
        ...
        // Create subfolder
        const testFolder = page
          .locator('a:has-text("e2e-test-folder"), .pk-finder-file:has-text("e2e-test-folder")')
          .first();
        if (await testFolder.isVisible()) {
          ...
          const subfolderInput = page
            .locator('input[placeholder*="Folder"], input[type="text"]')
            .last();
  ...
  test('Upload files', async ({ page }) => {
    ...
    if (!fs.existsSync(testImagePath)) {
      const imageBuffer = Buffer.from(
        '',
        'base64'
      );
  ...
  test('Delete files and folders', async ({ page }) => {
    ...
    const testItems = page
      .locator('.pk-finder-file, .uk-panel')
      .filter({ hasText: /e2e-test|test-/ });
    ...
    if ((await testItems.count()) > 0) {
      ...
      const deleteButton = page
        .locator('button:has-text("Delete"), button[title*="Delete"]')
        .first();
      if (await deleteButton.isVisible()) {
        ...
        const confirmButton = page
          .locator('button:has-text("Delete"), button:has-text("OK")')
          .last();
  ...
  test('Storage settings', async ({ page }) => {
    ...
    if (await settingsButton.isVisible()) {
      ...
      const maxSizeInput = page
        .locator('input[name*="max_size"], input[name*="upload_max"]')
        .first();
      ...
      const fileTypesInput = page
        .locator('input[name*="types"], input[name*="extensions"]')
        .first();
  ...
  test('Image preview and details', async ({ page }) => {
    ...
    if ((await imageFile.count()) > 0) {
      ...
      if (await previewPanel.isVisible()) {
        ...
        console.log(
          `✅ Image preview shows details (size: ${fileSize}, dimensions: ${dimensions})`
        );
      }
