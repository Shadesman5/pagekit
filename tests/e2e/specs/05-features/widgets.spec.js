...
test.describe.fixme('Pagekit Widget System', () => {
    ...
    if (await addButton.isVisible()) {
      ...
      if (await textWidgetOption.isVisible()) {
        ...
        const contentArea = page
          .locator('textarea[name="widget[content]"], .uk-htmleditor-content')
          .first();
        if (await contentArea.isVisible()) {
          await contentArea.fill(
            'This is a test widget created by E2E tests. It contains sample text content.'
          );
        }
  ...
  test('Create Menu Widget', async ({ page }) => {
    ...
    // Select Menu widget
    const menuWidgetOption = page
      .locator('.uk-modal')
      .locator('a:has-text("Menu"), button:has-text("Menu")')
      .first();
  ...
  test('Widget Visibility Rules', async ({ page }) => {
    ...
    if ((await widgetRow.count()) > 0) {
      ...
      const pagesInput = page
        .locator('input[name="widget[pages]"], textarea[name="widget[pages]"]')
        .first();
  ...
  test('Widget Frontend Display', async ({ page }) => {
    ...
    if (await textWidget.isVisible()) {
      ...
      await page
        .locator('textarea[name="widget[content]"]')
        .first()
        .fill('Test content visible on frontend');

      // Set to show on all pages
      const pagesInput = page
        .locator('input[name="widget[pages]"], select[name="widget[pages]"]')
        .first();
      if (pagesInput) {
