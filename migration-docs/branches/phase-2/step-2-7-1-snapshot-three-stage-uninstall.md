...
| `app/installer/src/Package/Snapshot/SnapshotStore.php` | `reserve()` claims the id by `mkdir` (fails if the directory is already there). `Filesystem::makeDir()` is not used here — it reports an existing directory as made. Collision retries the next id; a path that still does not exist afterwards is an unwritable store (removal aborted). |
...
| `app/installer/app/lib/uninstall.vue` | Confirm and success copy (and the Snapshots link) turn on `keepsSnapshots`. Default `false`. No-store confirm is the danger line that nothing is put aside. |
