# Views &amp; Templating
<p class="uk-article-lead">While the controller handles the incoming request, the view is responsible for rendering the response. Pagekit ships with two templating engines: a plain PHP engine and the Twig engine.</p>

The view layer is designed for **strict Content Security Policy (CSP)** compatibility: templates do not use `eval()`, and configuration data is exposed to the browser via data-attributes rather than inline `<script>` blocks. Themes that follow this pattern run under a `default-src 'self'` policy without `'unsafe-inline'` or `'unsafe-eval'`.
...
You can also resolve the `view` service yourself and render a template file. This is useful when the view name is computed dynamically. The `hello:` prefix in the example below refers to a resource shorthand defined in the package's [module definition](modules.md#register-resource-shorthands).
...
class MyController
{
    ...
    public function anotherViewAction(): string
    {
        return $this->view->render('hello:views/view.php', ['id' => 1]);
    }
}
```
...
Pagekit's templates follow modern security best practices:

- **No inline scripts.** Configuration data is exposed via data-attributes on a known DOM node and read by external JavaScript files.
- **No `eval()`.** Template engines never evaluate strings as PHP or JavaScript.
- **CSP-friendly.** Themes run under a strict Content Security Policy without `'unsafe-inline'` or `'unsafe-eval'`.

When developing themes or extensions, avoid inline JavaScript execution. Pass configuration via data-attributes and load a small bootstrap script that reads them:

```html
<script src="app/assets/js/config-loader.js"></script>
<script src="your-script.js"></script>
```

Your JavaScript reads the configuration from the DOM:
