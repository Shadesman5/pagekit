...
Translatable strings are listed in `*.pot` files (e.g. `app/system/languages/messages.pot`). Translations are stored in `*.php` files (e.g. `app/system/languages/de_DE/messages.php`). Each locale folder contains two translation domains:

- `messages.php` — the default `messages` domain (UI strings shared between core and extensions).
- `validators.php` — the `validators` domain used for Symfony Validator constraint messages.

While `*.pot` files live in the repository, manual pull requests against translation files are rejected. All contributions go through Transifex (and the pot files are regenerated automatically — see step B).
