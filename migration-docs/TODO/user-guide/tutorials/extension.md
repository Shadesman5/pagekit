...
To limit the access to the admin area and mount this controller to an admin url, we use the attribute `#[Access(admin: true)]`. Attributes are PHP 8 native features placed directly above a class or method. Learn [more about attributes](http://pagekit.com/docs/developer/routing#attributes).
...
From the controller, we can access this configuration as a property of the module instance. Inject the `Application` container — or the more specific services you need — via the constructor.
...
class TodoController
{
    ...
    public function indexAction()
    {
        $module = $this->app->get('module')->get('todo');
...
Module config is persisted in the database. Default values from the `index.php` are merged with the stored values, so the controller always sees a complete configuration:

```php
$this->app->get('config')->get('todo')->set('entries', $entries);
```
