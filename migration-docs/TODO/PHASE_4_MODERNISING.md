...
- **Components**:
  ...
  - **Snapshot surface on the API and the console**: list / restore / purge as versioned resources and `snapshot:*` console commands over the same `PackageSnapshotter`, so a site whose panel is down still has a way to a snapshot. Error contract: the panel already returns `RestoreRefusedException`'s sentence in `message` — a recorded application version that is missing, not text, or different; a `packages.*` name whose version differs or that sits on only one side; and the MySQL sentences from `DatabaseRestorer::refusals()` (no table prefix, a copy name past 64 characters, a reserved `_r_` / `_b_` name, an inbound foreign key, the lock). A readable snapshot with neither `application` nor `applicationStored` is not a version refusal. Any other throwable stays the fixed line and the log. `SnapshotStore::get()` and `list()` omit both keys; the version is `application()` and whether a null is an older snapshot is `applicationWasStored()`. The versioned resources and the `snapshot:*` commands return that same sentence and read the version through those methods, so a surface that wraps every throwable as the fixed line hides the sentence the panel already shows. `refusals()` releases the lock and does not drop leftover copies or replace tables. An unreadable dump, and a dump that holds no tables, are not this refusal: `restore()` reports them after the package files are back and leaves the database as it was. The retention window belongs on the same surfaces: `snapshots.retention_days` is config of the `package` module (`PackageModule` reads it once at boot for the store, the snapshots page shows it read-only), and today nothing but that module's `@system_config` row sets it; a value stored under any other module's name — `installer`, where the key lived before — is read by nothing.
