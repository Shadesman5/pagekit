...
- **Tasks (identified from 2.1.1 review):**
  ...
  - **Translation-key extraction (optional, low priority):**
    ...
    - Extract catalogue ids that are not a string-literal argument of `__()` or `$trans()`. `UnsatisfiedRequirementException::messageId()` returns `Module "%depender%" requires "%required%", which is registered but disabled.`, and the enable action and `php pagekit enable` pass that return value into `__()`. The not-registered twin is a literal in `PackageArchive::unknownRequirement()`; the disabled sentence is not, so a regeneration that only walks literal arguments never writes it into `messages.pot` and every locale keeps the English sentence
