...
- **Provides:** the fault barrier, auto-disable and admin-notification seam that Snapshot & Three-Stage Uninstall (2.7.1), Module Dependency Integrity (2.7.2), Static Module Registration (2.7.3), Extension Packaging (2.8) and the Marketplace (5.6) all build on. Step 2.10.2 (Controller FQCN Autowiring) also waits on the static bridges removed here.
...
- **The fallback file is private state.** Preferred home: a dedicated directory under `tmp/` that is **not** `path.temp` and **not** `path.cache` — e.g. `tmp/system/` (same pattern as `tmp/snapshots/` in 2.7.1). Register a path key for it. Never under `storage/` (HTTP-reachable through the `public/` mount). Never in a location `doClearCache` (cache or temp option) sweeps — if a clear path could reach it, exclude it explicitly.
- **Recovery must survive a double fault.** Config rollback, migration rollback, fallback write and logging each have their own error handling. A throwable from a rollback or a fallback write must be logged and must not erase or replace the original failure; the admin/API surface must still report that the enable/install failed and that recovery may be partial.
...
- **Do not redesign module discovery here.** Wrapping registration in `\Throwable` (and deciding what "degraded" means for that window) is in scope; replacing `include index.php` discovery with a static manifest so inactive packages never execute PHP is Step **2.7.3**, not this step.
...
- **Which failure windows are in scope.** Registration (`include` of every `index.php` on disk, enabled or not), module load / `main()`, and lifecycle script execution are three distinct windows with different recovery options. State explicitly which ones the barrier covers and what happens in each. A fatal parse error / compile error in a `require`d file cannot be caught in userland — decide how that case is handled (or accepted and documented). **Anti-pattern:** a bare `try/catch` around `include` with no answer for partial registration state and no stated parse-error posture is not a complete design. **Out of this step:** replacing PHP-executed discovery with a static manifest (Step 2.7.3) — do not expand into that rewrite here; if the barrier leaves the registration window only partially covered, say so and point the remainder at 2.7.3.
- **Where the disable record lives.** Default to a dedicated `tmp/system/` (or equivalent under `tmp/` that is neither `path.temp` nor `path.cache`), with its own path key, created when missing, writable in the Docker dev stack, excluded from both `doClearCache` clear paths, and never under `storage/`. Confirm the chosen location against those constraints — do not leave "somewhere under `tmp/`" unspecified.
...
- **Install rollback:** which migration-rollback entry point (`rollback()` / `rollbackExtension()`) `PackageManager::enable()` should call on throwable, and how to avoid rolling back migrations that were already applied before this install attempt. **Double-fault:** if that migration rollback itself throws (DB gone mid-rollback, inconsistent schema, migrator error), state the behaviour — config rollback must still run, the original exception remains the reported cause, the rollback failure is logged separately, and the caller must not be left with an uncaught throwable from the recovery path alone.
...
- **(1) Barrier:** the goal is not "wrap everything in `try/catch`" — it is a defined degradation. Decide per window what "degraded but consistent" means, and make the theme fallback (`SystemModule.php:62-71`) and the new extension fallback behave coherently. Registration stays on today's `include index.php` discovery; harden that window inside the barrier, do not replace the discovery model (2.7.3).
- **(3) Auto-disable:** the DB write is immediate (`ConfigManager::set()`), so the disable can land in the failing request when the database is healthy. When it is not, the fallback file is the only record — it must be authoritative on the next boot, which means boot has to merge both sources *before* the extension list is used. Write it with the 2.6 primitive into the dedicated private path from the Principles (not `tmp/temp`, not `storage/`).
...
- **(5) Install rollback:** `PackageManager::enable()` already restores config state on throwable; the missing half is the migration rollback. Do not duplicate the existing rollback — extend it. Wrap migration rollback so a second throwable cannot unwind the outer catch as an unhandled recovery failure; log double-faults and keep the original enable/install error as the primary report.
...
- **Static module registration / discovery without PHP execution for inactive packages** → Step **2.7.3** (#266). Today's `ModuleManager::register()` still `include`s every on-disk `index.php` to learn that a package exists; moving identity/metadata into a static file (`module.json` / equivalent) and restricting PHP execution to enabled packages is the root fix for the registration window — it rewrites the module contract and belongs after the fault barrier, not inside it.
- **Extension package shape, prebuilt assets, author-side build preset, publishing package assets into `public/`** → Step 2.8 (package contract must absorb whatever 2.7.3 settles as the manifest shape).
- **Marketplace, catalogue, remote distribution, package signing / integrity checks** → Step 5.6.
- **Sandboxing enabled extension PHP** (restricted runtime, process isolation) — out of scope for this step and for Core fault isolation. Not deferred 2.x debt. Optional future candidate after marketplace + trust model: Phase 5 §5.6 *Future candidate* (Extension / Sub-Extension only, never Core).
...
- **This area starts from zero coverage.** `ModuleManager`, `PackageManager`, `PackageScripts` and the dumpers have no tests at all, so the barrier and auto-disable paths need tests written from scratch — a fixture extension that throws is the natural instrument. Cover at minimum: throwable in `main()`, throwable in a lifecycle script, DB write failure on auto-disable (fallback taken), fallback record honoured on the next boot, re-enable clearing both sources, and migration-rollback double-fault (rollback throws → original failure still reported, no uncaught recovery throwable).
...
- The fallback record lives under a dedicated private path (not `tmp/temp`, not `storage/`), is not reachable over HTTP, and survives a routine cache/temp clear.
...
- A failed install no longer leaves migrations applied: config state *and* schema are rolled back together. If migration rollback itself fails, the recovery path does not throw uncaught, both failures are logged, and the enable/install error remains reportable to the admin.
...
- Module discovery still uses today's `index.php` registration model — no static-manifest rewrite sneaks into this PR (that is 2.7.3).
...
- **The registration window must be decided, not redesigned.** `ModuleManager::register()` includes *every* `index.php` under `packages/*/*/` before anything is enabled, so "disable the extension" does not protect against a throwable at that level. Decide deliberately: barrier around each `include`, consult the disable record before including a known offender, and/or document residual risk for uncatchable parse errors. Do **not** replace discovery with a static manifest in this ticket — that is Step 2.7.3. Do **not** treat a bare `try/catch` with no partial-state story as done.
...
- **Fallback path is constrained, not open-ended.** Prefer `tmp/system/` (dedicated path key; not `path.temp` / `path.cache` / `storage/`). `doClearCache(['temp' => true])` empties `tmp/temp` at depth 0 and the installer triggers a cache clear — a routine admin action must not silently re-enable a broken extension.
- **`PackageManager::enable()` already has the throwable barrier and a config rollback.** Extend it for migrations; do not write a second rollback path next to it. Plan the double-fault case when `rollback()` / `rollbackExtension()` throws.
