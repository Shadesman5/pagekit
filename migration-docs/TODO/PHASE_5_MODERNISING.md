...
- **5.0.2 — Parent-aware PackageManager**: enable-guard (parent must be active), cascade on disable/uninstall, install-reason bookkeeping, rollback — builds on the existing transactional `enable()` and the pre-flight from Step 2.7.2. `disable()` and `uninstall()` refuse the whole call when an enabled module that `requiredBy` lists still requires a target, and a sibling included in that same call still counts while it is enabled, so a cascade switches the depender off in its own call first and only then the dependency. There is no flag that skips the blocker. `enable()` of a `pagekit-theme` writes `site.theme` over the previous value and does not ask that pre-flight about the theme it replaces; an extension that required the outgoing theme is auto-disabled on the next boot, and the themes page has no disable control, so that enable is how the panel leaves the active theme. A package-scoped restore, and a purge that drops the tables a module owns, are still unbuilt. Ownership is what the pre-flight reports (migrations, the module's own config row, its node types, and tables named with the installation prefix plus the module name). Without those operations a removal keeps dumping every prefixed table, and a purge keeps leaving the package's tables in place. Orphaned dependencies stay a report until install-reason bookkeeping can tell a dependency the operator enabled from one that was only pulled in.
...
- **Promotion**: move the optional system modules out of the hardcoded `require` list in `app/system/index.php` into the activation registry, so a single list answers "what is switched on". A name that stays in that `require` is in the always-loaded closure of `system`: after the activity policy it stays active when it is absent from `extensions`, and the removal pre-flight does not report it as an orphan. Listing it in the registry without taking it out of `require` does not make it deactivatable. `feed` and `markdown` live in `app/modules` because that same list loads them, together with comment, captcha, finder, info and mail. Moving the directory while the require entry stays leaves the module always loaded. Deciding the tier, taking the name off that list, moving the directory and rewriting the static manifest from **2.7.3** are one change. `debug` requires `system/user`; a library module that loads a CMS module loses that require here, and the name stays.
