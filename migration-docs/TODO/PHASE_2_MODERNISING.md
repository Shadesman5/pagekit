...
- **Tasks (draft)**:
  ...
  - **Flip the residual `phpunit.xml.dist` deprecation/notice gates** (re-homed here from the now-complete Steps 2.1.4 / 2.1.5): `failOnDeprecation`, `failOnPhpunitDeprecation` and `failOnNotice` are still `"false"`. Step 2.1.9 already cleared the PHP-level StreamWrapper `$context` deprecation and flipped the warning/risky gates; what remains is ~2 PHPUnit-internal metadata deprecations (plus a re-verify of notices across the 8.2/8.3 matrix, and revisiting `SYMFONY_DEPRECATIONS_HELPER=disabled`). Clear the residue, then flip all three to `"true"` so deprecations/notices fail the run like warnings/risky already do. The in-file `phpunit.xml.dist` comment now points its ownership here.
