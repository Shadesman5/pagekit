...
- **What**:
  ...
  4. `installer` keeps the wizard (`Installer`, `InstallerController`, `TablePrefix`, requirements, install scripts, `StorageLink`, `Helper\InstallerIO`, `Helper\Factory`) and the marketplace / self-update clients, which are on the removal path and are not touched here. `TablePrefix` is the install-time half of the reserved-namespace invariant the snapshot engine relies on (an installable prefix leads with a letter, the restore's `_r_`/`_b_` markers lead with an underscore); after the move the two halves live in two modules, and the Snapshot suite's assertion that the markers fail `TablePrefix::refusal()` is the one place they meet — it crosses the boundary as a test and stays.
  5. Every call site follows the move, and so do the PSR-4 map, the PHPStan baseline (24 entries name `app/installer`), the CS-Fixer config, `.gitignore`, the three boot files and the build scripts. More than 70 PHP files name the namespace. No `class_alias`, no second namespace, no dual service registration.
...
- **What**:
  1. Delete the runtime install path: the Composer helper (install, uninstall, `isInstalled`, update, the Composer factory), the `artifact` repository over `path.artifact` / `tmp/packages`, the Composer branch of `PackageManager::removeFiles()`, the Composer bookkeeping in the snapshot engine (`installed.json` capture, `composer` metadata flag, `INSTALLED_FILE` accessor) and the `installed.json` version fallback in `PackageManager` — with their tests. The dead Composer imports in the console `build` / `update` commands go with it. Remove `composer/composer` from the runtime `require`; nothing else in the tree needs `composer/semver` (`LifecycleRunner` compares with `version_compare`) — a constraint syntax for the manifest is a decision for **2.8**, not a reason to keep the library. The PHPStan baseline entries that covered the helper's Composer 1 calls go with the file; the helper is not ported to the Composer 2 API first.
