...
- **Out of scope**: dividing `PackageManager` itself — its size is a separate problem and a relocation does not split a file; deleting or rewriting the marketplace / self-update clients; any behaviour change whatsoever, since this step relocates code; the static manifest discovery will need (**2.7.3**); `vendor/` at the repo root (**2.7.4**).
