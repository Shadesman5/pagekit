...
- **What**:
  ...
  4. `installer` keeps the wizard (`Installer`, `InstallerController`, requirements, install scripts, `StorageLink`, `Helper\InstallerIO`, `Helper\Factory`) and the marketplace / self-update clients, which are on the removal path and are not touched here.
...
- **What**:
  ...
  - **Pre-flight before restore, too**: a snapshot restore is a whole-database revert with DDL recreate, so any core or extension updated since the snapshot gets its schema and `packages.*` version key reverted while its code stays new — core re-migrates on the next admin request, extensions only on `enable()`. Compare the snapshot's recorded application version and the dump's `packages.*` keys against the running installation and warn or refuse by name. Needs `PackageSnapshotter::details()` to record the application version (metadata only; the dump format does not change).
