...
- **Forward**: Update rollback uses this restore → Step 2.9; the full PHPUnit MySQL leg becomes required → Step 2.11
...
- **Forward**: Delete the runtime Composer path → Step 2.7.1c; declared `require` edges must match imports → Step 2.7.2; static manifests → Step 2.7.3; `vendor/` at the repo root → Step 2.7.4; comment-prose sweep → Step 2.11
...
- **What**:
  1. Static package identity/metadata file (e.g. `module.json` beside `composer.json`, or a constrained subset of an existing manifest) carrying at least name and the fields registration needs before load — no PHP evaluation to learn that a package exists. The `autoload` map is one of those fields: with the installed-package Composer overlay gone (**2.7.1c**) it is the only autoload source, so registration must apply it for enabled packages before their entry point runs
...
- **Depends on**: Step 2.4.1 (`public/` is the only served tree, so a private directory beside it is unreachable over HTTP); Step **2.7.1c** (the snapshot store changes shape there — move it afterwards, once). Independent of 2.7.4.
...
- **What**:
  ...
  3. `tmp/` holds only what a clear may take — `cache/`, `temp/`, upload staging, `logs/` (a lost log costs diagnosis, not data). A path-posture test replaces `SnapshotPathWiringTest`: every `path.*` the boot names lies under exactly one of `tmp/`, `data/`, `storage/`, and `data/` lies outside `public/` and `storage/`.
...
- **Candidate**: Enable the PHP-CS-Fixer `nullable_type_declaration_for_default_null_value` rule so an implicit-nullable parameter cannot return: the baseline carries no `parameter.implicitlyNullable` entry any more, and the console IO adapter that held the last three (`app/package/src/Helper/InstallerIO.php`, deleted with the runtime Composer path in **2.7.1c**) already declares explicit `?Type` (no new tooling — CS-Fixer already runs in CI).
