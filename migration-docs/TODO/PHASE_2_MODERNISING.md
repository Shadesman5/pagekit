...
- **What**:
  ...
  - **Declared edges match imports**: every core module's `require` names the modules whose classes its code imports — attribute classes such as `Pagekit\User\Attribute\Access` on controllers included, which resolve lazily and so never fail at load time — checked by a test that reads the imports and the manifests together. Failing closed is only worth having on a graph that is complete; an edge somebody forgot to declare is a dependency the rules cannot see.
...
## Step 2.7.2b: Boot Module Name

- **Depends on**: Step 2.7.2 (the foundation classes live in `kernel`; the composition root only boots the stack).
- **Goal**: The composition root's directory and its module name are both `boot`. It requires `kernel` and the stack. The `Application` class stays in `kernel`.
- **Why**: A directory named `application` that no longer contains `Application` sends the next reader to the wrong module. Static discovery (**2.7.3**) records module paths, so a rename after that rewrites every manifest.
- **What**: Rename the module and the directory together. Every `require` of `application` becomes `boot`. No second name remains.
- **Out of scope**: The `Pagekit\` namespace (**4.7**). Moving modules out of `app/system` (**5.0**). Splitting the first-run wizard from the updater (**2.9**).
- **Sequencing**: after **2.7.2**, before **2.7.3**.
- **Risk**: Low — one module name and the `require` lists that name it.

---

## Step 2.7.3: Static Module Registration

- **Depends on**: Step 2.7 (fault barrier / auto-disable seams), Step 2.7.2 (dependency graph already fail-closed against registered names), Step 2.7.2b (discovery records the composition root as `boot`).
...
- **What**:
  1. Static package identity/metadata file (e.g. `module.json` beside `composer.json`, or a constrained subset of an existing manifest) carrying at least name and the fields registration needs before load — no PHP evaluation to learn that a package exists. The `autoload` map is one of those fields: the module manifest is the only autoload source, so registration must apply it for enabled packages before their entry point runs. The archive install reads `name` and `autoload` out of the module `index.php` without executing it — `Pagekit\Package\Archive\PackageArchive` parses the file and accepts only a literal `return` array — and that parser is the one reader to retarget: the static file becomes its source, so the archive check and boot discovery read the same fields from the same file, and a manifest the install accepted is one discovery can register. No second reader, no `index.php` fallback once the static file is required
